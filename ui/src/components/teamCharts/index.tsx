...
export default function TeamCharts({
    ...
    datasets: [
      {
        ...
        data: [totalStats?.wins ?? 0, totalStats?.losses ?? 0],
  ...
  const groupedBarData = {
    ...
    datasets: [
      {
        label: "Vitórias",
        data: [totalStats?.winsHome ?? 0, totalStats?.winsRoad ?? 0],
        backgroundColor: ["green", "blue"],
      },
      {
        label: "Derrotas",
        data: [totalStats?.lossesHome ?? 0, totalStats?.lossesRoad ?? 0],
        backgroundColor: ["red", "#784008"],
      },
    ],
  };
  const pieData = {
    ...
    datasets: [
      {
        label: "Distribuição",
        data: [
          totalStats?.winsHome ?? 0,
          totalStats?.winsRoad ?? 0,
          totalStats?.lossesHome ?? 0,
          totalStats?.lossesRoad ?? 0,
        ],
        backgroundColor: ["green", "blue", "red", "brown"],
      },
    ],
  };
  const radarData = {
    labels: [
      "Pontos Casa",
      "Pontos Sofridos Casa",
      "Pontos Fora",
      "Pontos Sofridos Fora",
    ],
    datasets: [
      {
        label: "Média de Pontos",
        data: [
          totalStats?.scorePerGameHome ?? 0,
          totalStats?.opponentScorePerGameHome ?? 0,
          totalStats?.scorePerGameRoad ?? 0,
          totalStats?.opponentScorePerGameRoad ?? 0,
        ],
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: "Vitórias",
        data: team?.games?.map((game) => (game.result === "W" ? 1 : 0)) ?? [],
        borderColor: "green",
        fill: false,
      },
      {
        label: "Derrotas",
        data: team?.games?.map((game) => (game.result === "L" ? 1 : 0)) ?? [],
        borderColor: "red",
        fill: false,
      },
    ],
